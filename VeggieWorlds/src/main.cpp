...
void togglePneumatic(pros::controller_digital_e_t btn, pros::ADIDigitalOut& piston, bool& state, bool& pressed){
	if (controller.get_digital(btn)) {
        if (!pressed) {
			 state = !state; piston.set_value(state); pressed = true;
		}
    } else {
        pressed = false;
    }
...
void opcontrol() {
    ...
    while (true) {
		...
		// ---------------------------
     	...
     	// WING TOGGLE
		togglePneumatic(pros::E_CONTROLLER_DIGITAL_A,wing, wing_state, wing_pressed);

		// MIDSCORE TOGGLE
		togglePneumatic(pros::E_CONTROLLER_DIGITAL_Y, midscore, midscore_state, midscore_pressed);

    
        // limiter TOGGLE
     	togglePneumatic(pros::E_CONTROLLER_DIGITAL_B, limiter, limiter_state, limiter_pressed);
		 
		// matchloader TOGGLE
		togglePneumatic(pros::E_CONTROLLER_DIGITAL_X, matchloader, matchloader_state, matchloader_pressed);

		// ----------------------------
		// Haptics
		//-----------------------------
		if (midscore_state && !haptics_cooldown) {
			controller.rumble(".");  haptics_cooldown = true; 
		} else if (limiter_state && !haptics_cooldown) {
			controller.rumble("--"); haptics_cooldown = true; 
		} else {
